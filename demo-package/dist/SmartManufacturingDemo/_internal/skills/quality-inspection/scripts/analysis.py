...
质量巡检 Skill - 分析算法脚本
=============================
功能：加载质量巡检标准知识库，比对质检数据与阈值，输出正常/预警/严重/紧急状态与告警清单。

用法示例：
    python analysis.py --data ../data/quality_data.json --out ../output/analysis_result.json
"""
import argparse
import json
import os
...
ALERT_HINTS = {
    "预警": "48小时内复核质量趋势；若持续越界，组织QC分析与工艺排查。",
    "严重": "当班隔离该批次产品，通知QC主管并暂停该线放行，排查根因。",
    "紧急": "立即停线扣留相关批次，启动质量事故响应，同步工程与客户质量部。",
}
...
def main():
    parser = argparse.ArgumentParser(description="质量巡检 - 分析算法")
    parser.add_argument("--data", default=os.path.join(SKILL_ROOT, "data", "quality_data.json"),
                        help="质检数据JSON路径")
    ...
    if not os.path.exists(args.data):
        print("[quality-inspection/analysis] 错误：未找到质检数据文件，请先运行 data_fetch.py。")
        sys.exit(1)
    ...
    s = result["summary"]
    print(f"[quality-inspection/analysis] 分析完成 → {args.out}")
    print(f"[quality-inspection/analysis] 状态：正常 {s['正常']} / 预警 {s['预警']} / 严重 {s['严重']} / 紧急 {s['紧急']}")
    print(f"[quality-inspection/analysis] 共发现告警 {len(result['alerts'])} 条")
