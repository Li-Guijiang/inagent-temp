...
def _fetch_from_mcp(device_ids=None):
    """从本地产线动态数据服务（app.py 端口5000）拉取实时工艺数据。

    调用 https://127.0.0.1:5000/api/process，返回结构与 `_fetch_demo` 一致。
    ...
    try:
        ...
        if not devices:
            raise ValueError("动态服务返回空工艺单元列表")
        return devices
    except Exception as e:
        print(f"[process-optimization/data_fetch][warn] 动态数据服务不可用({e})，本次回退到 demo 模拟数据")
