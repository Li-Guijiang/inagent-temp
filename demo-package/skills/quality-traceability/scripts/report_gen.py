...
质量追溯 Skill - 报告生成脚本
=============================
功能：基于分析结果生成质量追溯日报、告警文件，并同步到 Excel 追溯台账。

用法示例：
    python report_gen.py
    python report_gen.py --ledger ../output/traceability_ledger.xlsx
...
def _compose_alert_files(result, report_dir):
    ...
    for status, alerts in alerts_by_status.items():
        ...
        fname = os.path.join(report_dir, f"traceability_alert_{status}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md")
...
def _gen_daily_report(result, report_dir):
    ...
    s = result["summary"]
    total = sum(s.values())
    summary_text = f"当前共分析 {total} 个追溯单元，正常 {s['正常']} 项；预警 {s['预警']} 项、严重 {s['严重']} 项、紧急 {s['紧急']} 项。"
    if result["alerts"]:
        summary_text += f"共触发 {len(result['alerts'])} 项追溯告警，详见明细与告警文件。"
    else:
        summary_text += "本次追溯体系全部正常，无需处置。"
    recs = []
    if s["紧急"]:
        recs.append("- **紧急项**：立即冻结受影响批次销售发运，启动全面追溯核查。")
    if s["严重"]:
        recs.append("- **严重项**：当班排查断链点，核实受影响批次范围。")
    if s["预警"]:
        recs.append("- **预警项**：核查数据链路，补充缺失记录防止断链扩大。")
    if not recs:
        recs.append("- 全部正常，维持现行追溯体系管理计划。")
    recs.append("- 建议将连续告警项纳入追溯体系评审与数据治理专项。")

    html = _render(
        DAILY_TMPL,
        generated_at=result["generated_at"],
        scope_desc=f"全部追溯单元（数据时间 {result.get('data_time', '未知')}，来源 {result.get('data_source', '未知')}）",
        ...
        device_rows="\n".join(rows) if rows else "（无追溯项）",
        recommendations="\n".join(recs),
    )
    fname = os.path.join(report_dir, f"traceability_report_{datetime.now().strftime('%Y%m%d')}.md")
    with open(fname, "w", encoding="utf-8") as f:
        f.write(html)
    return fname
...
def _update_ledger(result, ledger_path):
    try:
        ...
        print("[quality-traceability/report_gen] 提示：未安装 openpyxl，跳过 Excel 台账同步。")
        return False

    os.makedirs(os.path.dirname(ledger_path), exist_ok=True)
    headers = ["日期", "设备代码", "设备名称", "所属企业", "追溯参数", "单位",
               "当前值", "状态", "正常限", "超限次数", "说明"]
...
def main():
    parser = argparse.ArgumentParser(description="质量追溯 - 报告生成")
    ...
    parser.add_argument("--ledger", default=os.path.join(OUTPUT_DIR, "traceability_ledger.xlsx"),
                        help="Excel台账路径")
    args = parser.parse_args()

    if not os.path.exists(args.result):
        print("[quality-traceability/report_gen] 错误：未找到分析结果，请先运行 data_fetch.py + analysis.py。")
        sys.exit(1)
    ...
    print(f"[quality-traceability/report_gen] 日报已生成 → {daily}")

    alert_files = _compose_alert_files(result, args.report_dir)
    for af in alert_files:
        print(f"[quality-traceability/report_gen] 告警文件已生成 → {af}")
    ...
    if ok:
        print(f"[quality-traceability/report_gen] Excel 台账已同步 → {args.ledger}")
