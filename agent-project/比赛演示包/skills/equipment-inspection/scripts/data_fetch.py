...
def _fetch_from_mcp(device_ids=None):
    """从本地产线动态数据服务（app.py 端口5000）拉取实时设备数据。

    调用 https://127.0.0.1:5000/api/equipment，返回结构与 `_fetch_demo` 一致。
    服务不可用（未启动）时自动回退到 demo 模拟数据并告警。
    """
    import json as _json
    import urllib.request as _urlreq

    endpoint = "http://127.0.0.1:5000/api/equipment"
    if device_ids:
    ...
    try:
        ...
        if not devices:
            raise ValueError("动态服务返回空设备列表")
        return devices
    except Exception as e:
        print(f"[data_fetch][warn] 动态数据服务不可用({e})，本次回退到 demo 模拟数据")
