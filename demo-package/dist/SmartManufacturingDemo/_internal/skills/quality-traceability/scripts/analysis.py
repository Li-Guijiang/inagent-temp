...
质量追溯 Skill - 分析算法脚本
=============================
功能：加载追溯标准知识库，比对追溯指标与阈值，输出正常/预警/严重/紧急状态与追溯告警清单。

用法示例：
    python analysis.py --data ../data/traceability_data.json --out ../output/analysis_result.json
"""
import argparse
import json
import os
...
ALERT_HINTS = {
    ...
    "严重": "当班排查追溯断链点，核实相关批次影响范围，必要时通知客户质量部。",
    "紧急": "立即冻结受影响批次销售与发运，启动全面追溯核查，排除产品召回风险。",
}
...
def main():
    parser = argparse.ArgumentParser(description="质量追溯 - 分析算法")
    parser.add_argument("--data", default=os.path.join(SKILL_ROOT, "data", "traceability_data.json"),
                        help="追溯数据JSON路径")
    ...
    if not os.path.exists(args.data):
        print("[quality-traceability/analysis] 错误：未找到追溯数据文件，请先运行 data_fetch.py。")
        sys.exit(1)
    ...
    s = result["summary"]
    print(f"[quality-traceability/analysis] 分析完成 → {args.out}")
    print(f"[quality-traceability/analysis] 状态：正常 {s['正常']} / 预警 {s['预警']} / 严重 {s['严重']} / 紧急 {s['紧急']}")
    print(f"[quality-traceability/analysis] 共发现告警 {len(result['alerts'])} 条")
