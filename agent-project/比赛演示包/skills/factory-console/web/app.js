...
/* 实时刷新：每 5 秒重拉当前视图，数据来自动态服务（端口5000），体现动态变化 */
function refreshView() {
  if (!document.hidden) {
    if (currentView === "alerts") loadAlerts();
    else if (currentView === "quality") loadQuality();
    else if (currentView === "energy") loadEnergy();
    else if (currentView === "home") loadHomeTodo();
  }
}
...
async function loadAlerts() {
  ...
  $("#alerts-updated").textContent = (r.live ? "🟢 实时动态 · 每5秒自动刷新 · " : "") + "更新于 " + (r.updated_at || "-");
  const s = r.summary || {};
...
async function loadQuality() {
  ...
  $("#quality-updated").textContent = (r.live ? "🟢 实时动态 · 每5秒自动刷新 · " : "") + "更新于 " + (r.updated_at || "-");
  const s = r.summary || {};
...
async function loadEnergy() {
  ...
  if (!r.ok) { box.innerHTML = `<div class="ok-strip">${r.msg}</div>`; return; }
  $("#energy-updated").textContent = (r.live ? "🟢 实时动态 · 每5秒自动刷新 · " : "") + "更新于 " + (r.updated_at || "-");
  const s = r.summary || {};
